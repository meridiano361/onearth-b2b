...
export default function AdminSondaggiModificaPage({ surveyId }: { surveyId: string }) {
  ...
  useEffect(() => {
    (async () => {
      const { survey }: { survey: SurveyFull } = await fetch(`/api/admin/surveys/${surveyId}`).then((r) => r.json());
      if (!survey) { setLoading(false); return; }
      setHasResponses((survey._count?.responses ?? 0) > 0);
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <Link href={`/admin/sondaggi/${surveyId}`} className="text-gray-400 hover:text-primary transition-colors p-1 -ml-1">
            <ArrowLeft size={18} />
          </Link>
          <div>
            <p className="label-luxury text-accent mb-0.5">Admin · Sondaggi</p>
