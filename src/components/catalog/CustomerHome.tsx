...
function SpotlightCard({ product }: { product: Product }) {
  ...
  return (
    <div className="flex-none w-36 flex flex-col bg-white rounded-lg border border-border overflow-hidden">
      {/* Image */}
      <Link href={`/catalog/${product.id}`} className="block relative">
      ...
      <div className="p-2 flex flex-col flex-1">
        <Link href={`/catalog/${product.id}`}>
          <p className="text-xs text-primary leading-snug line-clamp-2 min-h-[2.5rem] mb-2">{product.name}</p>
        </Link>
        <button
          onClick={handleAdd}
          className={cn(
  ...
  return (
    <div className="min-h-screen bg-cream">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        <section>
          <p className="font-display text-sm sm:text-xl text-primary font-light tracking-wide mb-4 whitespace-nowrap overflow-hidden text-ellipsis">
            {th('discoverTitle')}
          </p>

          {/* Black social box */}
          <div className="bg-black rounded-xl p-5">
            {/* Row 1: on-earth.it + IG + FB */}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full px-4 py-2.5 text-sm text-white bg-white/10 border-2 border-green-400 rounded-lg hover:bg-white/20 transition-colors"
            >
              <Mic size={16} />
              Podcast MATERIA
            </a>
